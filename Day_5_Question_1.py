...
def move(number_to_move, from_row, to_row):
    i = 1
    while i <= int(number_to_move):
        element_to_move = state[from_row].pop()
        state[to_row].append(element_to_move)
        i += 1

def execute_commands(commands):
    for command in commands:
        move(number_to_move=command[0], from_row=int(command[1]), to_row=int(command[2]))
